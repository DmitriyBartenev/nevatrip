...
const printTicket = () => {
  ...
  el.output.innerHTML = `
  <p>${inputValue} билета по маршруту ${selectedDirection} стоимостью ${cost} руб.</p>
  <p>Это путешествие займет у вас ${travelTimeMinutes} минут.</p>
  <p>Теплоход отправляется в ${selectedDirection === 'B → A' ? departureBA : departureAB}. ${selectedDirection === 'A → B → A' ? 'Обратно в ' + departureBA : ''}</p>
  `;
};
